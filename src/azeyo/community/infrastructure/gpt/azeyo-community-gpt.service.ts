...
export class AzeyoCommunityGptService {
  ...
  async generatePost(commentCount: number, recentPosts: { category: string; title: string }[] = [], postTime?: Date): Promise<GptGeneratedPost> {
    ...
    let reviewed = parsed;
    try {
      const reviewPrompt = this.buildReviewPrompt(type, category, commentCount, postTime);
  ...
  private buildPrompt(type: AZEYO_COMMUNITY_POST_TYPE, category: AZEYO_COMMUNITY_CATEGORY, commentCount: number, recentPosts: { category: string; title: string }[] = [], postTime?: Date): string {
    ...
    return `너는 "아재요"라는 기혼 남성 커뮤니티에 글 쓰는 30~40대 남자야.

## 페르소나
- 나이: 30~40대 기혼 남성
- 근무: 월~금 9시~18시 직장인 (일반 사무직/회사원)
- 출퇴근: 편도 1시간 (왕복 2시간, 대중교통 or 자차)
- 자녀: 아이 1명 (어린이집~초등 저학년)
- 월급: 약 300만원 (세후, 빠듯한 살림)
- 생활패턴: 아침 7시쯤 출발 → 9시 출근 → 18시 퇴근 → 19시쯤 귀가, 주말에는 가족과 시간
- 이 페르소나의 시각에서 자연스럽게 공감되는 글을 써야 함 (월급 빠듯, 출퇴근 피곤, 육아 병행 등)
...
## JSON 형식으로만 응답
{
  ...
  private buildReviewPrompt(type: AZEYO_COMMUNITY_POST_TYPE, category: AZEYO_COMMUNITY_CATEGORY, commentCount: number, postTime?: Date): string {
    const baseTime = postTime || new Date();
    const kstMs = baseTime.getTime() + 9 * 60 * 60 * 1000;
    const kstNow = new Date(kstMs);
    const kstHour = kstNow.getUTCHours();
    ...
    const dayNames = ['일요일', '월요일', '화요일', '수요일', '목요일', '금요일', '토요일'];
    const dayOfWeek = dayNames[kstDay];
    const timeSlot = kstHour < 1 ? '밤늦은 시간' : kstHour < 6 ? '새벽' : kstHour < 9 ? '이른 아침' : kstHour < 12 ? '오전' : kstHour < 14 ? '점심시간' : kstHour < 18 ? '오후' : kstHour < 22 ? '저녁' : '밤늦은 시간';
    const isVote = type === AZEYO_COMMUNITY_POST_TYPE.VOTE;
    const isHolidaySeason = kstMonth === 2 || kstMonth === 9;

    return `너는 기혼 남성 커뮤니티 게시글 검수자야. 아래 JSON을 검토하고 문제가 있으면 수정해서 같은 JSON 형식으로 응답해. 문제없으면 그대로 응답해.
모든 시간/요일 판단은 한국시간(KST, UTC+9) 기준이야.

현재 (한국시간 KST 기준): ${kstMonth}월 ${kstNow.getUTCDate()}일 ${dayOfWeek} ${timeSlot} (${isWeekend ? '주말' : '평일'})
...
2. 시간대 부적합: ${isWeekend ? '주말인데 출근/퇴근/회사/직장/동료/팀장/상사/업무/야근/학교/어린이집/유치원/점심(회사) 관련 내용이 있으면 집/가족/외출/쉬는 상황으로 반드시 수정. "일하다가", "동료", "같이 일하는" 같은 표현도 포함됨' : kstHour < 9 || kstHour >= 22 ? '새벽/밤인데 출근/퇴근/회사/팀장/상사/업무/야근 내용이 있으면 집에서 쉬는 상황으로 수정' : '현재 시간대에 맞지 않는 상황이 있으면 수정'}
