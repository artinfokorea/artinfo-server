...
import {
  IOnchurchSmsTemplateRepository,
  ONCHURCH_SMS_TEMPLATE_REPOSITORY,
} from '@/onchurch/master/domain/repository/onchurch-sms-template.repository.interface';
...
@Injectable()
export class OnchurchCreateSmsTemplateUseCase {
  constructor(
    @Inject(ONCHURCH_USER_REPOSITORY) private readonly userRepository: IOnchurchUserRepository,
    @Inject(ONCHURCH_SMS_TEMPLATE_REPOSITORY) private readonly templateRepository: IOnchurchSmsTemplateRepository,
  ) {}

  async execute(userId: number, params: { name: string; subject: string; content: string }): Promise<OnchurchSmsTemplate> {
...
@Injectable()
export class OnchurchListSmsTemplatesUseCase {
  constructor(
    @Inject(ONCHURCH_USER_REPOSITORY) private readonly userRepository: IOnchurchUserRepository,
    @Inject(ONCHURCH_SMS_TEMPLATE_REPOSITORY) private readonly templateRepository: IOnchurchSmsTemplateRepository,
  ) {}

  async execute(userId: number): Promise<OnchurchSmsTemplate[]> {
    await assertMaster(this.userRepository, userId);
...
@Injectable()
export class OnchurchDeleteSmsTemplateUseCase {
  constructor(
    @Inject(ONCHURCH_USER_REPOSITORY) private readonly userRepository: IOnchurchUserRepository,
    @Inject(ONCHURCH_SMS_TEMPLATE_REPOSITORY) private readonly templateRepository: IOnchurchSmsTemplateRepository,
  ) {}
