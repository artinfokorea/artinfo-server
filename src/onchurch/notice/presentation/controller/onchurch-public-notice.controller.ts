...
import { OnchurchListPublicNoticeCategoriesUseCase } from '@/onchurch/notice/application/usecase/onchurch-notice-category.usecase';
import { OnchurchPublicNoticeListResponse, OnchurchNoticeCategoryListResponse } from '@/onchurch/notice/presentation/dto/response/onchurch-notice.response';

@RestApiController('/onchurch/sites', 'Onchurch Public Site')
export class OnchurchPublicNoticeController {
  constructor(
    private readonly listPublicUseCase: OnchurchListPublicNoticesUseCase,
    private readonly listCategoriesUseCase: OnchurchListPublicNoticeCategoriesUseCase,
  ) {}
  ...
  @RestApiGet(OnchurchNoticeCategoryListResponse, { path: '/:slug/notice-categories', description: '공개 - 공지 활성 카테고리 목록' })
  async listCategories(@Param('slug') slug: string) {
    return new OnchurchNoticeCategoryListResponse(await this.listCategoriesUseCase.execute(slug));
  }
}
